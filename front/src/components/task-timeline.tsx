...
const STATUS_INDEX: Record<string, number> = {
  ...
  completed: 4, awaiting_verification: 5, verified: 6, paid: 7, cancelled: -1,
};

export function TaskTimeline({ status }: { status: string }) {
  const idx = STEPS.findIndex(s => s.key === status);
  const currentIdx = idx >= 0 ? idx : (STATUS_INDEX[status] ?? 0);
  // Terminal state: the whole flow is finished, so every step (including the
  // last) reads as completed — shown in success green, no pulsing "current".
  const complete = status === "paid" || status === "PAID";

  const filledCls = complete ? "border-success bg-success text-white" : "border-primary bg-primary text-primary-foreground";
  const lineCls = complete ? "bg-success" : "bg-primary";
          ...
          <li key={step.key} className="flex flex-1 items-start gap-3 sm:flex-col sm:items-center sm:text-center">
            <div className="flex flex-col items-center sm:w-full">
              <div className="flex w-full items-center">
                {i > 0 && <div className={cn("hidden h-0.5 flex-1 sm:block", done || current ? lineCls : "bg-border")} />}
                <div className={cn(
                ...
                {i < STEPS.length - 1 && <div className={cn("hidden h-0.5 flex-1 sm:block", done ? lineCls : "bg-border")} />}
              </div>
